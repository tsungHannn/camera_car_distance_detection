...
if __name__=='__main__':
    ...
    parser.add_argument("--maxv", help='Maximum row index of the image which is considered', default=1536)  
    parser.add_argument("--tao", help='CATS parameter tao', default=3)  
    parser.add_argument("--eps", help='CATS parameter eps', default=200)  
    ...
    for i in range(start,end):
        ...
        for j in range(len(radarTrack[i])):
            # radarTrack: ID, time, x, y, z, vx, vy, vz
            ID, radarTime, x, y, z = radarTrack[i][j][0], radarTrack[i][j][1], radarTrack[i][j][2], radarTrack[i][j][3], radarTrack[i][j][4]
            vx, vy, vz = radarTrack[i][j][5], radarTrack[i][j][6], radarTrack[i][j][7]
            if sqrt(vx**2+vy**2+vz**2)<0.01:
                continue
            now={'frame':i,'pos':np.array([x,y,z]),'time':radarTime,'v':np.array([vx, vy, vz])}

            radarTrackID[ID]=np.array([now]) if not ID in radarTrackID.keys() else np.append(radarTrackID[ID],now)
        
        # print("#"*10)
        # print(radarTrack[i][0][1])
        # input()
